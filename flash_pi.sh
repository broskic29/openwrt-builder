...
while getopts "d:f:c:i:ozh" opt; do
  case "$opt" in
    d) DISK_NAME="$OPTARG" ;;
    ...
    z) UNZIP=1 ;;
    h)
      echo "Usage: $0 -d disk -f flash_disk -c config -i image [-o, -z]. Example: ./flash_pi -d /dev/disk4 -f /dev/rdisk4 -c working_config_with_packages -o"
      exit 0
      ;;
    *)
      echo "Invalid option"
